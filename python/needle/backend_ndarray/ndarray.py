...
class NDArray:
    ...
    def numpy(self):
        ...
        return self.device.to_numpy(self._handle, self.shape, self.strides, self._offset)

    def is_compact(self):
        """Return true if array is compact in memory and internal size equals product
        of the shape dimensions"""
        return self._strides == self.compact_strides(self._shape) and prod(self.shape) == self._handle.size
    ...
    def compact(self):
        ...
        if self.is_compact():
            ...
            self.device.compact(self._handle, out._handle, self.shape, self.strides, self._offset)
            return out
    ...
    def reshape(self, new_shape):
        ...
        if prod(self.shape) != prod(new_shape):
            raise ValueError(
                "product of current shape is not equal to the product of the "
                "new shape, or if the matrix is not compact."
            )
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        # BEGIN YOUR SOLUTION
        for d in zip(self.shape, new_shape):
            if d[0] != 1 and d[0] != d[1]:
                raise ValueError(
                    f"""new_shape's elements must be the
                    same as the original shape, except for dimensions in the self where
                    the size = 1 (which can then be broadcast to any size).
                    shape: {self.shape} new_shape: {new_shape}"""
                )
        new_strides = list(self.strides)
        for i, s in enumerate(self.shape):
            assert s == 1 or s == new_shape[i]
            if s == 1:
                new_strides[i] = 0
    ...
    def __getitem__(self, idxs):
        ...
        if not isinstance(idxs, tuple):
            idxs = (idxs,)
        idxs = tuple(
            [self.process_slice(s, i) if isinstance(s, slice) else slice(s, s + 1, 1) for i, s in enumerate(idxs)]
        )
        ...
        for i in range(len(idxs)):
            new_shape.append(math.ceil((idxs[i].stop - idxs[i].start) / idxs[i].step))
            new_offset += idxs[i].start * self.strides[i]
            new_strides.append(self.strides[i] * idxs[i].step)
        return NDArray.make(
            tuple(new_shape), strides=tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset
        )
        # raise NotImplementedError()
        # END YOUR SOLUTION
    ...
    def __mul__(self, other):
        return self.ewise_or_scalar(other, self.device.ewise_mul, self.device.scalar_mul)

    __rmul__ = __mul__

    def __truediv__(self, other):
        return self.ewise_or_scalar(other, self.device.ewise_div, self.device.scalar_div)
    ...
    def __matmul__(self, other):
        ...
        if hasattr(self.device, "matmul_tiled") and all(d % self.device.__tile_size__ == 0 for d in (m, n, p)):
            ...
            return out.permute((0, 2, 1, 3)).compact().reshape((self.shape[0], other.shape[1]))

        else:
            out = NDArray.make((m, p), device=self.device)
            self.device.matmul(self.compact()._handle, other.compact()._handle, out._handle, m, n, p)
            return out
    ...
    def flip(self, axes):
        ...
        ret = NDArray.make(
            shape=self.shape, strides=tuple(new_strides), device=self.device, handle=self._handle, offset=offset_sum
        )
        return ret.compact()
        # raise NotImplementedError()
        # END YOUR SOLUTION
