...
def _unpack_params_named(prefix: str, value: object) -> List[Tensor]:
    if isinstance(value, Parameter):
        return {prefix: value}
    elif isinstance(value, Module):
        ...
        for k, v in value.items():
            ...
            params.update(_unpack_params_named(p, v))
        return params
    elif isinstance(value, (list, tuple)):
        params = {}
...
class Module:
    ...
    def named_parameters(self, prefix=None) -> Dict[str, Tensor]:
        """Return the list of parameters in the module."""
        return _unpack_params_named(prefix, self.__dict__)
...
class Linear(Module):
    def __init__(self, in_features, out_features, bias=True, device=None, dtype="float32"):
        ...
        if bias:
            self.bias = Parameter(
                init.kaiming_uniform(
                    out_features, 1, shape=(out_features, 1), device=device, dtype=dtype, requires_grad=True
...
class BatchNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            return x_normed * self.weight.reshape((1, self.dim)).broadcast_to(x.shape)
            ...
            x_normed = (x - self.running_mean.reshape((1, self.dim)).broadcast_to(x.shape)) / (
                self.running_var.reshape((1, self.dim)).broadcast_to(x.shape) + self.eps
            ) ** 0.5
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.weight.reshape((x.shape[0], 1)).broadcast_to(x.shape) * normed
        +self.bias.reshape((x.shape[0], 1)).broadcast_to(x.shape)
        # END YOUR SOLUTION
...
class RNN(Module):
    ...
    def forward(self, X, h0=None):
        ...
        for t, x in enumerate(Xs):
            hiddens = []
            for i, model in enumerate(self.rnn_cells):
                x = model(x, hs[i])
                hiddens.append(x)
            out.append(x)
            hs = hiddens
...
class LSTMCell(Module):
    def __init__(self, input_size, hidden_size, bias=True, device=None, dtype="float32"):
        ...
        self.W_ih = Parameter(
            init.rand(
                il, 4 * hl, low=-1 / hl**0.5, high=1 / hl**0.5, device=device, dtype=dtype, requires_grad=True
            )
        )
        self.W_hh = Parameter(
            # NOTE: hl, 4*hl
            init.rand(
                hl, 4 * hl, low=-1 / hl**0.5, high=1 / hl**0.5, device=device, dtype=dtype, requires_grad=True
            )
        )
        if bias:
            self.bias_ih = Parameter(
                init.rand(
                    4 * hl, low=-1 / hl**0.5, high=1 / hl**0.5, device=device, dtype=dtype, requires_grad=True
                )
            )
            self.bias_hh = Parameter(
                init.rand(
...
class LSTM(Module):
    ...
    def forward(self, X, h=None):
        ...
        for t, x in enumerate(Xs):
            hiddens = []
            cells = []
            for i, model in enumerate(self.lstm_cells):
                x, c_out = model(x, (hs[i], cs[i]))
                hiddens.append(x)
                cells.append(c_out)
            out.append(x)
            hs = hiddens
