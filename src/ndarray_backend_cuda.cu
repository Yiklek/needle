...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec strides,
                              size_t offset) {
  /**
  ...
  if (gid < size) {
    ...
    out[gid] = a[pos + offset];
  }

  /// END YOUR SOLUTION
}

void Compact(const CudaArray& a, CudaArray* out, const std::vector<int32_t>& shape, const std::vector<int32_t>& strides,
             size_t offset) {
  /**
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, CudaVec strides,
  ...
  if (gid < size) {
    ...
    out[pos + offset] = a[gid];
  }
}

void EwiseSetitem(const CudaArray& a, CudaArray* out, const std::vector<int32_t>& shape,
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape, CudaVec strides,
                                    size_t offset) {
  /**
   * The CUDA kernel for the compact opeation.  This should effectively map a single entry in the
   * non-compact input a, to the corresponding item (at location gid) in the compact array out.
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray* out, const std::vector<int32_t>& shape,
                   const std::vector<int32_t>& strides, size_t offset) {
  /**
...
__global__ void EwiseTanhKernel(const scalar_t* a, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[gid] = (exp(a[gid]) - exp(-a[gid])) / (exp(a[gid]) + exp(-a[gid]));
}
...
__global__ void ReduceMaxKernel(scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
    ...
    for (size_t i = 0; i < reduce_size; i++) m = max(m, a[gid * reduce_size + i]);
    out[gid] = m;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
    scalar_t m = 0;
    for (size_t i = 0; i < reduce_size; i++) m += a[gid * reduce_size + i];
    out[gid] = m;
  }
}
...
PYBIND11_MODULE(ndarray_backend_cuda, m) {
  ...
  py::class_<CudaArray>(m, "Array")
      ...
      .def("ptr", &CudaArray::PtrAsInt);

  // return numpy array, copying from CPU
  m.def("to_numpy",
        [](const CudaArray& a, const std::vector<size_t>& shape, std::vector<size_t> strides, size_t offset) {
          std::vector<size_t> numpy_strides = std::move(strides);
          std::transform(numpy_strides.begin(), numpy_strides.end(), numpy_strides.begin(),
                         [](size_t& c) { return c * ELEM_SIZE; });

          // copy memory to host
          auto* host_ptr = static_cast<scalar_t*>(std::malloc(a.size * ELEM_SIZE));
  ...
  m.def("from_numpy", [](const py::array_t<scalar_t>& a, CudaArray* out) {
