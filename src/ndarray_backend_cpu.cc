...
struct AlignedArray {
  explicit AlignedArray(const size_t size) {
    int ret = posix_memalign(reinterpret_cast<void**>(&ptr), ALIGNMENT, size * ELEM_SIZE);
    if (ret != 0) throw std::bad_alloc();
    this->size = size;
  }
  ~AlignedArray() { free(ptr); }
  [[nodiscard]] size_t PtrAsInt() const { return reinterpret_cast<size_t>(ptr); }
  scalar_t* ptr;
  size_t size;
};
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape, std::vector<int32_t> strides,
             size_t offset) {
  /**
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape, std::vector<int32_t> strides,
                  size_t offset) {
  /**
...
void EwiseTanh(const AlignedArray& a, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    out->ptr[i] = (std::exp(a.ptr[i]) - std::exp(-a.ptr[i])) / (std::exp(a.ptr[i]) + std::exp(-a.ptr[i]));
  }
}
...
inline void AlignedDot(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ out) {
  /**
  ...
  a = static_cast<const float*>(__builtin_assume_aligned(a, TILE * ELEM_SIZE));
  b = static_cast<const float*>(__builtin_assume_aligned(b, TILE * ELEM_SIZE));
  out = static_cast<float*>(__builtin_assume_aligned(out, TILE * ELEM_SIZE));
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  for (int i = 0; i < out->size; i++) {
    size_t left = i * reduce_size;
    size_t right = (i + 1) * reduce_size;
    // size_t right = std::min((i + 1) * reduce_size, a.size);
    out->ptr[i] = *std::max_element(a.ptr + left, a.ptr + right);
  }
  /// END YOUR SOLUTION
}
...
void ReduceSum(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (int i = 0; i < out->size; i++) {
    ...
    size_t right = (i + 1) * reduce_size;
    // size_t right = std::min((i + 1) * reduce_size, a.size);
    out->ptr[i] = std::accumulate(a.ptr + left, a.ptr + right, static_cast<scalar_t>(0));
  }
  /// END YOUR SOLUTION
}
...
PYBIND11_MODULE(ndarray_backend_cpu, m) {
  ...
  py::class_<AlignedArray>(m, "Array")
      ...
      .def("ptr", &AlignedArray::PtrAsInt)
      .def_readonly("size", &AlignedArray::size);

  // return numpy array (with copying for simplicity, otherwise garbage
  // collection is a pain)
  m.def("to_numpy",
        [](const AlignedArray& a, const std::vector<size_t>& shape, std::vector<size_t> strides, size_t offset) {
          std::vector<size_t> numpy_strides = std::move(strides);
  ...
  m.def("from_numpy", [](const py::array_t<scalar_t>& a, AlignedArray* out) {
